...
use crate::{
    context::AppContext,
    extensions::{AnyhowErrorToStringChain, AppContextExt, ClientBuilderExt},
...
/// API 客户端：短超时 + 有界重试。
///
/// 超时 3 秒是刻意的——页面请求本身很轻，慢到这个程度基本就是被墙或
/// 上游挂了，继续等只是浪费用户时间。重试总时长限制在 5 秒内，
/// 避免一个死掉的域名把整批搜索拖住。
fn create_api_client(app: &AppContext) -> ClientWithMiddleware {
...
/// 让 `AnyhowErrorToStringChain` 在这个模块里可用（`create_api_client` 未用到，
/// 但代理设置失败时的日志走的是同一套格式化）。
#[allow(dead_code)]
fn _assert_ext_in_scope(err: &anyhow::Error) -> String {
    err.to_string_chain()
}