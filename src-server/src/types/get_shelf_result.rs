...
/// 书架页（`/users-users_fav-page-N-c-SHELF.html`）。
...
impl GetShelfResult {
    pub fn from_html(ctx: &AppContext, html: &str) -> anyhow::Result<GetShelfResult> {
        let document = Html::parse_document(html);

        let mut comics = Vec::new();
        for comic_div in document.select(&Selector::parse(".asTB").to_anyhow()?) {
            // 单本解析失败不影响整页：可能是广告位之类的非漫画条目。
            if let Ok(comic) = ComicInShelf::from_div(ctx, &comic_div) {
                comics.push(comic);
            }
        }
...
impl ComicInShelf {
    pub fn from_div(ctx: &AppContext, div: &ElementRef) -> anyhow::Result<ComicInShelf> {
